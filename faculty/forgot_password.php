...
<html lang="en">
<head>
	<title>OES - Faculty Forgot Password</title>
	...
	<div class="container">
		<form class="form" action="forgot_password.php" method="POST">
			...
			<div class="form-group">
				<label for="txb_FacultyEmail">Faculty Email Id</label>
				<input type="email" class="form-control" name="txb_FacultyEmail" id="txb_FacultyEmail" placeholder="Enter Faculty Email Id" required oninvalid="this.setCustomValidity('Please Enter valid email')">
			</div>
			...
			<?php
				if(isset($_POST['btn_request_password']))
				{
					...
					if($response->success)
					{
						$email=$_POST['txb_FacultyEmail'];
						$hash=md5($email);
						require_once '../globle/mail.php';
						mysqli_query($con,"update tbl_faculty set faculty_login_pwd='$pwdhash' where faculty_hash='$hash'") or die("Error To reset password");
					}
					else
					{
