...
<html lang="en">
<head>
	<title>OES - Student Forgot Password</title>
	...
	<div class="container">
		<form class="form" action="forgot_password.php" method="POST">
			...
			<div class="form-group">
				<label for="txb_StudentEmail">Student Email Id</label>
				<input type="email" class="form-control" name="txb_StudentEmail" id="txb_StudentEmail" placeholder="Enter Student Email Id" required oninvalid="this.setCustomValidity('Please Enter valid email')">
			</div>
			...
			<?php
				if(isset($_POST['btn_request_password']))
				{
					...
					if($response->success)
					{
						$email=$_POST['txb_StudentEmail'];
						$hash=md5($email);
						require_once '../globle/mail.php';
						mysqli_query($con,"update tbl_student set student_login_pwd='$pwdhash' where student_hash='$hash'") or die("Error To reset password");
					}
					else
					{
