...
<html lang="en">
<head>
	<title>OES - Admin Forgot Password</title>
	...
	<div class="container">
		<form class="form" action="forgot_password.php" method="POST">
			...
			<div class="form-group">
				<label for="txb_AdminEmail">Admin Email Id</label>
				<input type="email" class="form-control" name="txb_AdminEmail" id="txb_AdminEmail" placeholder="Enter Admin Email Id" required>
			</div>
			...
			<?php
				if(isset($_POST['btn_request_password']))
				{
					...
					if($response->success)
					{
						$email=$_POST['txb_AdminEmail'];
						$hash=md5($email);
						require_once '../globle/mail.php';
						mysqli_query($con,"update tbl_admin set admin_login_pwd='$pwdhash' where admin_hash='$hash'") or die("Error To reset password");
					}
					else
					{
